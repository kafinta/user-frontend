<template>
  <button :disabled="disabled" :class="disabled ? 'cursor-not-allowed text-accent-400' : is_active ? 'bg-white' : 'hover:bg-accent-500 duration-500 ease-in-out'" class="group flex items-center px-8 w-full py-3 gap-3">
    ...
    <div :class="disabled ? 'text-accent-400 ' : is_active ? 'font-medium text-secondary' : 'font-normal text-[#909090] group-hover:text-accent-300' " class="text-base 2xl:text-lg duration-500 ease-in-out">{{menu_text}}</div>
  </button>
</template>
