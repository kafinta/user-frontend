    ...
    <div class="md:flex block w-full gap-5">
      ...
      <div class="w-full">
        <div class="flex justify-between items-center w-full">
          <UiButtonsTab @clicked="filter_drawer = true" class="md:hidden flex gap-3 items-center">
            <p class="text-base">Filters</p>
            <UiIconsFilter class="w-5 group-hover:text-secondary" />
          </UiButtonsTab>
          <UiTypographyP class="hidden md:block">My Products</UiTypographyP>
        ...
        <ul v-if="products" class="grid lg:grid-cols-2 gap-6 place-items-center mt-6">
          <li v-for="item in 4" class="flex gap-4 border items-center border-accent-100 rounded-lg w-full p-2 h-full">
            <div class="bg-accent-200 rounded-md aspect-[3/4] w-1/3 h-full"></div>
