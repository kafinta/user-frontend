    ...
    <Container>
      <ul class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-8">
        <li v-if="locationsLoaded" v-for="item in locations" :key="item.id">
          <UiCards @clicked="query.location=item.name; $router.push({name: 'marketplace-products', query})" :title="item.name" :backgroundImagePath="config.public.base_url + item.image" class="w-full"/>
        </li>
